...
from src.llm.prompts import prompt_analytic, prompt_finance
...
if __name__ == "__main__":
    ...
    analytic = Agent(
        name=prompt_analytic["name"],
        prompt=prompt_analytic["system"],
        model=model
    )
    finance = Agent(
    ...
    messages_analytic = filter_by_author("message", prompt_analytic["name"], train_history.messages)
    scores_analytic = filter_by_author("score", prompt_analytic["name"], train_history.messages)
    ...
    answers = filter_by_author("message", prompt_analytic["name"], test_history.messages)
    print(predictor_analytic.predict(answers))

    predict_model = make_predict(train_history, {
        prompt_analytic["name"]: prompt_analytic["system"],
        prompt_finance["name"]: prompt_finance["system"]
    })

    model_analytic = predict_model(prompt_analytic["name"])
    ...
    volumes_train_text = compute_subspace_intersection_v2(
        filter_by_author("message", prompt_analytic["name"], train_history.messages),
